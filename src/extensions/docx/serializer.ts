...
export class DocxSerializerState {
  ...
  math(latex: string, opts: MathOpts = { inline: true }) {
    if (opts.inline || !opts.numbered) {
      this.current.push(new DocxMath({ children: [new MathRun(latex)] }))
      return
    }
  ...
  image(
    ...
    let alignment: Lowercase<keyof typeof AlignmentType>
    switch (align) {
      case 'right':
    ...
    this.addParagraphOptions({
      alignment: alignment as unknown as 'right' | 'left' | 'center',
    })
  }
